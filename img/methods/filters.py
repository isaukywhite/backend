...
def show_yiq(path):
    ...
    cv.imshow('Imagem em YIQ', yiq_matrix)
    cv.waitKey(0)
...
def convert_to_rgb(y_matrix, i_matrix, q_matrix):
    ...
    hp.show_image('RGB para YIQ para RGB', rgb_img)
...
def negative(path, option):
    ...
    if option == '1':
        ...
        hp.show_image('Imagem Negativa', negativeImage)
        ...
        cv.imshow('Imagem Negativa', negativeImage)
        cv.waitKey(0)
        cv.destroyAllWindows()
...
def brightness_handler(path, measure, option):
    ...
    hp.show_image('Imagem ./brilho', brightnessImage)
...
def thresholding_y(path, choose, measure):
    ...
    if choose == "1":
        ...
        cv.imshow('Medida Y limiar', threY)
        cv.waitKey(0)
        cv.destroyAllWindows()
    else:
        count = 0
        ...
        for i in range(row):
            for j in range(col):
                if y_matrix[i, j] < count:
                    ...
                    y_matrix[i, j] = 255
        threY = cv.merge([y_matrix, i_matrix, q_matrix])
        cv.imshow('Media Y limiar', threY)
        cv.waitKey(0)
        cv.destroyAllWindows()
