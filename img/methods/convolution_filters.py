...
def median_filter(path, mask):
    ...
    for i in range(row):
        for j in range(col):
            red_aux = np.sort(
                np.array(r_ext[i:i+row_mask, j:j+col_mask]).flatten())
            green_aux = np.sort(
                np.array(g_ext[i:i+row_mask, j:j+col_mask]).flatten())
            blue_aux = np.sort(
                np.array(g_ext[i:i+row_mask, j:j+col_mask]).flatten())
    ...
    hp.show_image('Filtro mediano', median_image)
...
def sobel_filter(path, mask):
    ...
    hp.show_image('Filtro Sobel', sobel_image)
